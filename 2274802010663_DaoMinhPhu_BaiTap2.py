...
def show_admin_page(connection):
    ...
    global current_connection  # Sử dụng biến toàn cục để lưu trữ kết nối hiện tại
    current_connection = connection  # Lưu kết nối hiện tại vào biến toàn cục

    login_frame.pack_forget()
    admin_frame = tk.Frame(win, padx=10, pady=10)
    admin_frame.pack()

    welcome_label = tk.Label(admin_frame, text=f"Chào Mừng Đến Trang Quản Trị", font=("Arial", 16))
    welcome_label.pack(pady=20)

    insert_data_frame = tk.Frame(admin_frame)
    insert_data_frame.pack(pady=10)

    tk.Label(insert_data_frame, text="User Name:").grid(row=0, column=0, padx=5, pady=5)
    global input_name
    input_name = tk.Entry(insert_data_frame)
    input_name.grid(row=0, column=1, padx=5, pady=5)

    tk.Label(insert_data_frame, text="User Password:").grid(row=1, column=0, padx=5, pady=5)
    global input_password
    input_password = tk.Entry(insert_data_frame)
    input_password.grid(row=1, column=1, padx=5, pady=5)

    tk.Label(insert_data_frame, text="User Role:").grid(row=2, column=0, padx=5, pady=5)
    global input_role
    input_role = tk.Entry(insert_data_frame)
    input_role.grid(row=2, column=1, padx=5, pady=5)
    ...
    tables_label = tk.Label(admin_frame, text="Danh sách các bảng và nội dung trong cơ sở dữ liệu:", font=("Arial", 12))
